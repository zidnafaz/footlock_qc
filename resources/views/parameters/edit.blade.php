...
@section('content')
<div class="mb-4">
    <h2><i class="bi bi-pencil-square"></i> Edit Parameter</h2>
</div>
        ...
        <form action="{{ route('parameters.update', $parameter->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="aspect" class="form-select @error('aspect') is-invalid @enderror" required>
                    ...
                    <option value="Visual" {{ old('aspect', $parameter->aspect) == 'Visual' ? 'selected' : '' }}>Visual</option>
                    <option value="Dimensional" {{ old('aspect', $parameter->aspect) == 'Dimensional' ? 'selected' : '' }}>Dimensional</option>
                    <option value="Packaging" {{ old('aspect', $parameter->aspect) == 'Packaging' ? 'selected' : '' }}>Packaging</option>
                </select>
                @error('aspect')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="sub_parameter" class="form-control @error('sub_parameter') is-invalid @enderror"
                       value="{{ old('sub_parameter', $parameter->sub_parameter) }}"
                       required>
                @error('sub_parameter')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <select name="is_active" class="form-select @error('is_active') is-invalid @enderror" required>
                    <option value="Yes" {{ old('is_active', $parameter->is_active) == 'Yes' ? 'selected' : '' }}>Aktif</option>
                    <option value="No" {{ old('is_active', $parameter->is_active) == 'No' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('is_active')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
